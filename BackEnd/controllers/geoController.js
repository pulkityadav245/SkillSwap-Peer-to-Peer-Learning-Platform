...
//  - Find nearby users by distance (Haversine formula)
// ─────────────────────────────────────────────
...
exports.getNearbyUsers = async (req, res) => {
    try {
        ...
        const me = await User.findById(req.user.id).select("coordinates location");
        ...
        }).select("name email location coordinates trustScore averageRating");
...
exports.getNearbySkills = async (req, res) => {
    try {
        ...
        const skills = await Skill.find({ userId: { $ne: req.user.id } })
            .populate("userId", "name location coordinates trustScore");
...
// ─────────────────────────────────────────────
//  GEOCODE endpoint (helper for frontend)
// ─────────────────────────────────────────────
